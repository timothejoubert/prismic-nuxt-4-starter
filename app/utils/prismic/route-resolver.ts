...
import {
    ...
    type PrismicDocumentRoute,
} from '~~/shared/prismic-routes'

export function isPrismicDocumentRoute(
    route: object,
): route is PrismicDocumentRoute {
...
export function getDocumentRoutePath(documentType: PrismicDocumentType) {
    const currentRoute = prismicDocumentRoutes.find(
        (route) => route.type === documentType,
    )
